  ...
  if (now - lastCaptureTime >= CAPTURE_COOLDOWN_MS) {
    ...
    if (distMasuk > MASUK_MIN_CM && distMasuk < MASUK_MAX_CM) {
      ...
      if (result == "ORGANIC") {
        ...
        // Backend tidak yakin dengan klasifikasi (confidence rendah / tidak
        // ada objek terdeteksi). Servo TIDAK digerakkan sama sekali supaya
        // sampah tidak salah masuk bin. Sampah tetap di posisi masuk,
        // menunggu dicoba lagi pada siklus loop berikutnya.
...
String waitForClassifyResult() {
  ...
  while (millis() - startTime < 15000) {   // timeout 15 detik
    if (Serial.available()) {
      ...
      if (response == "ORGANIC" || response == "NON_ORGANIC" || response == "UNKNOWN") {
        return response;
      }
